...
export default Route.extend({
	...
	actions: {
		addTo: function(source, destination, items) {
			items = items.filter(el=>source.includes(el))
			destination.pushObjects(items)
			source.removeObjects(items)

			let model = this.modelFor(this.routeName)
			let tmp = model.get('dispoItemsIds_')
			model.set('dispoItemsIds_', model.get('includedItemsIds_'))
			model.set('includedItemsIds_', tmp)
		}
	}
});
